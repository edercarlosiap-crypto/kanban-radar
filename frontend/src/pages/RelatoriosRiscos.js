...
export default function RelatoriosRiscos() {
  ...
  useEffect(() => {
    const carregarRelatorio = async () => {
      try {
        setCarregando(true);
        const response = await relatoriosAPI.riscos(filtros);
      ...
      <div style={styles.mainContent}>
        <div style={styles.header}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <LogoImage size={32} />
            <h1>Relatorios - Riscos</h1>
          </div>
        </div>

        <RelatoriosTabs ativo="/relatorios/riscos" />
          ...
          <div style={styles.card}>
            <table style={styles.table}>
              ...
              <tbody>
                {itens.map((item) => (
                  <tr key={item.id}>
                    <td style={styles.td}>{item.acao}</td>
                    <td style={styles.td}>{item.responsavel}</td>
                    <td style={styles.td}>{item.equipe}</td>
                    <td style={styles.td}>{item.camada}</td>
                    <td style={styles.td}>{item.concluirAte}</td>
                    <td style={styles.td}>{item.diasRestantes}</td>
