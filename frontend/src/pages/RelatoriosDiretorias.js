...
export default function RelatoriosDiretorias() {
  ...
  useEffect(() => {
    const carregarRelatorio = async () => {
      try {
        setCarregando(true);
        const response = await relatoriosAPI.diretorias(filtros);
        setDiretorias(response.data.diretorias || []);
        setErro('');
      } catch (e) {
      ...
      <div style={styles.mainContent}>
        <div style={styles.header}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <LogoImage size={32} />
            <h1>Relatorios - Diretorias</h1>
          </div>
        </div>

        <RelatoriosTabs ativo="/relatorios/diretorias" />
        ...
        {carregando ? (
          ...
          <div style={styles.card}>
            <table style={styles.table}>
              <thead>
                <tr>
                  <th style={styles.th}>Diretoria</th>
                  <th style={styles.th}>Total</th>
                  <th style={styles.th}>Atrasados</th>
                  <th style={styles.th}>% No Prazo</th>
                </tr>
              </thead>
              <tbody>
                {diretorias.map((item) => (
                  <tr key={item.diretoria}>
                    <td style={styles.td}>{item.diretoria}</td>
                    <td style={styles.td}>{item.total}</td>
                    <td style={styles.td}>{item.atrasados}</td>
                    <td style={styles.td}>{item.percentualNoPrazo}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
