...
export default function RelatoriosTimeline() {
  ...
  useEffect(() => {
    const carregarRelatorio = async () => {
      try {
        setCarregando(true);
        const response = await relatoriosAPI.timeline(filtros);
      ...
      <div style={styles.mainContent}>
        <div style={styles.header}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <LogoImage size={32} />
            <h1>Relatorios - Timeline</h1>
          </div>
        </div>

        <RelatoriosTabs ativo="/relatorios/timeline" />
            ...
            <div style={{ height: 360 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={timeline}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="iniciados" name="Iniciados" fill="#2563eb" />
                  <Bar dataKey="finais" name="Finais previstos" fill="#22c55e" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
